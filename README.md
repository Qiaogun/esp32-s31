...
`ota_check` downloads and validates the backend OTA manifest without flashing. It can also be triggered remotely with a queued `ota_check:manifest` command. `ota_update` downloads the manifest, verifies the firmware URL against the manifest `sha256` and `size`, applies the verified `firmware_url` through `esp_https_ota`, reports OTA state back to `/api/v1/ota/report`, and reboots after a successful update. Remote flashing is intentionally explicit: queue `ota_update:apply` only when you want the device to verify, flash, and reboot.
